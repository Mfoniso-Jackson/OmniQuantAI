...
- finite, revocable, quota-limited automated contract strategy authorization with durable local audit records
...
OpenClaw installation and updates must use `scripts/update_openclaw_skills.sh`. Production mode is pinned to the official AI Wars repository `https://github.com/weex-labs/weex-agent-skills-ai-wars`, `main`, and approved commit `1e2fb63731729c8039fa6a97108a5b0d0f6c5d7e`; it verifies the checkout before changing skill symlinks and restores the previous version if OpenClaw checks fail. Non-official repository or branch overrides require explicit `--dev`.
...
| `Automated Authorization` | Stable strategy identity, bounded FUTURES authorization, deterministic quota checks, AI-log-bound submission, events, recovery, and snapshots. | Saved profile + real contract only |
...
Automatic authorization is an explicit integration for user-maintained strategies and is limited to `FUTURES + real trading + saved profile`. Register a stable strategy identity, request the five-dimensional scope (module, symbols, per-leg conservative U maximum, cumulative U maximum, and validity), inspect the pending request, and grant the exact request with `--confirm-live`. Granting changes local authorization state only; it does not place an order.
...
Futures order quantity semantics are resolved before risk, valuation, and submission. Use `quantity_unit=BASE_ASSET` for base-asset input or `quantity_unit=CONTRACTS` for explicit contract counts; the latter requires official `contractVal` and is converted exactly once. The REST request, risk preview, and conservative valuation use the resulting base-asset quantity.

Use `scripts/weex_auto_trade.py` as the production facade. While an authorization is active, eligible single-leg and conditional orders can skip per-order human confirmation, but they still pass official risk/fact completeness, conservative valuation, scope, quota, state, and AI Log checks. Every automatic WEEX write must include a validated log whose output matches the final order. Missing or mismatched logs stop before request preparation. `transaction.place_orders_batch` has no AI Wars log contract and returns `AI_LOG_CONTRACT_UNAVAILABLE` before state reservation, providers, or network access.

The owner-only local state keeps authorization requests, grants, Decimal quota, usages, order ownership, events, managed snapshots, and restore gates. Explicit rejections release reserved quota; uncertain results and primary-trade-success/AI-log-upload-failure results remain `REVIEW_REQUIRED`, preserve known order IDs, and are never retried automatically. Revocation, expiry, retirement, snapshot restore, and reconciliation do not cancel, amend, recreate, or retry WEEX orders. See [Script operations](references/script-operations.md) for the command surface and recovery flow.

## Saved Profile Setup

Private account and trading operations require a saved profile.

Resolve the saved profile from the user's explicit current choice, an unambiguous choice already made in the conversation, or the configured default. If multiple usable profiles remain and none is uniquely selected, show profile display names only and ask the user to choose; do not guess from list order, IDs, notes, or name similarity.
...
- An order preview containing `position_id` represents an exact full-position close. The guard revalidates the fresh position identity, direction, symbol, and quantity before routing it to `transaction.close_positions`; partial quantities and ambiguous or missing position IDs fail closed.
