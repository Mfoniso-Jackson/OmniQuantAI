...
def complete_futures_symbol_facts(*, maker_fee="0", taker_fee="0"):
    return {
        "quoteAsset": "USDT",
        "marginAsset": "USDT",
        "contractVal": "1",
...
def complete_ai_log(output=None):
    return {
        "stage": "Strategy Generation",
        "model": "gpt-5-2026-08-19",
        "input": {"messages": [{"role": "user", "content": "place the bounded futures order"}]},
        "output": dict(output or {}),
        "explanation": "The saved strategy selected this bounded order from the supplied futures context.",
    }
...
class AutoTradeAuthorizationRequestTests(unittest.TestCase):
    def test_scope_rejects_non_futures_trade_type(self) -> None:
        state_module = load_state_module()
        scope = complete_scope(trade_types=["SPOT"])

        with self.assertRaisesRegex(ValueError, "trade_types only accepts FUTURES"):
            state_module.normalize_scope(scope)
    ...
    def test_ensure_reuses_pending_request_for_same_owner_and_normalized_scope(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            first = state.ensure_authorization(
                strategy_id=strategy["strategy_id"],
                scope={
                    "trade_types": ["FUTURES", "FUTURES", "FUTURES"],
...
class AutoTradeAmountTests(unittest.TestCase):
    @unittest.skip("AI Wars automated trading is FUTURES-only")
    ...
    @unittest.skip("legacy cash-notional quota scenario is outside the FUTURES-only scope")
...
class AutoTradeCliTests(unittest.TestCase):
    ...
    def test_cli_authorization_flow_returns_detailed_confirmation_and_events(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(confirmation["trade_types"], ["FUTURES"])
...
class AutoTradeFacadeProductionBoundaryTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            runtime = SimpleNamespace(
                ...
                facts_provider=lambda leg: {
                    "timestamp_ms": now_ms,
                    "symbol": complete_futures_symbol_facts(),
                    "reduce_only_proven": False,
                    "degraded_reasons": [],
                },
                submitter=lambda operation_key, legs, **kwargs: [
                    {
    ...
    def test_submit_auto_ai_log_upload_failure_requires_review_and_preserves_order(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            boundary = runtime_module.OfficialApiBoundary(profile_name=profile.name)
            boundary._client = Mock(return_value=(weex_contract_api, Mock()))
            runtime = runtime_module.OfficialAutoTradeRuntime(
                profile_name=profile.name,
                api=boundary,
                risk_aggregator=Mock(),
            )
            runtime.risk_payload_provider = lambda leg: {
                "partial": False,
                "degraded_reasons": [],
                "constraints": [],
                "account_snapshot": complete_spot_account_snapshot(),
            ...
            runtime.risk_evaluator = lambda payload: {
                "alerts": [],
                "rule_version": "fixture-v1",
            }
            runtime.facts_provider = lambda leg: {
                "timestamp_ms": now_ms,
                "symbol": complete_futures_symbol_facts(),
                "reduce_only_proven": False,
                "degraded_reasons": [],
            ...
            contract_result = {
                "businessOk": True,
                "status": 200,
                "result": {"orderId": "weex-ai-log-failed-1", "success": True},
                "tradeExecuted": True,
                "aiLogUploadFailed": True,
                "retryTradeSafe": False,
                "aiLogFailureReason": "upload denied",
            }

            with patch.object(
                weex_contract_api,
                "execute_endpoint_payload",
                return_value=(weex_contract_api.EXIT_CODE_AI_LOG_FAILED, contract_result),
            ):
                result = facade.execute(
                    "submit-auto",
                    {
                        ...
                        "strategy_id": strategy["strategy_id"],
                        "authorization_id": authorization["authorization_id"],
                        "idempotency_key": "facade-ai-log-upload-failed",
                        "operation_key": "transaction.place_order",
                        "orders": [order],
                        "ai_log": complete_ai_log(order),
                    },
            ...
            leg = result["legs"][0]
            self.assertEqual(leg["weex_order_id"], "weex-ai-log-failed-1")
            self.assertEqual(leg["error_code"], "AI_LOG_UPLOAD_FAILED")
            self.assertTrue(leg["trade_executed"])
            self.assertTrue(leg["ai_log_upload_failed"])
            self.assertFalse(leg["retry_trade_safe"])
            self.assertEqual(leg["authorization_quota"]["reserved_amount_u"], "1")
            review_event = next(
                event
                for event in state.list_events(strategy_id=strategy["strategy_id"])
                if event["event_type"] == "USAGE_REVIEW_REQUIRED"
            )
            self.assertEqual(review_event["payload"]["error_code"], "AI_LOG_UPLOAD_FAILED")

    def test_submit_auto_ai_log_mismatch_fails_before_runtime_and_quota_reservation(self) -> None:
        import weex_auto_trade_runtime as runtime_module
        import weex_contract_api
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            client = SimpleNamespace(prepare_request=Mock(), send=Mock())
            boundary = runtime_module.OfficialApiBoundary(profile_name=profile.name)
            boundary._client = Mock(return_value=(weex_contract_api, client))
            runtime = runtime_module.OfficialAutoTradeRuntime(
                profile_name=profile.name,
                api=boundary,
                risk_aggregator=Mock(),
            ...
            runtime.risk_payload_provider = risk_provider
            runtime.risk_evaluator = Mock(
                return_value={"alerts": [], "rule_version": "fixture-v1"}
            )
            runtime.facts_provider = Mock(
                return_value={
                    "timestamp_ms": now_ms,
                    "symbol": complete_futures_symbol_facts(),
                    "reduce_only_proven": False,
            ...
            try:
                facade.execute(
                    "submit-auto",
                    {
                        ...
                        "strategy_id": strategy["strategy_id"],
                        "authorization_id": authorization["authorization_id"],
                        "idempotency_key": "facade-ai-log-mismatch",
                        "operation_key": "transaction.place_order",
                        "orders": [order],
                        "ai_log": complete_ai_log({**order, "symbol": "ETHUSDT"}),
                    },
                    confirm_live=True,
                )
            except cli_module.FacadeError as exc:
                error = exc

            self.assertIsNotNone(error)
            self.assertEqual(error.code, "AI_LOG_VALIDATION_FAILED")
            runtime_factory.assert_not_called()
            risk_provider.assert_not_called()
            runtime.risk_evaluator.assert_not_called()
            runtime.facts_provider.assert_not_called()
            boundary._client.assert_not_called()
    ...
    def test_submit_auto_manual_fallback_creates_bound_pending_intent(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(captured[0]["auto_fallback_operation_key"], "transaction.place_order")
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            result = facade.execute(
                "submit-auto",
                {
                    ...
                    "ai_log": complete_ai_log({"symbol": "BTCUSDT"}),
                },
                confirm_live=True,
            )
    ...
    def test_state_rejects_same_named_forged_evidence_object(self) -> None:
        state_module = load_state_module()
        with tempfile.TemporaryDirectory() as tempdir:
            state = state_module.AutoTradeState(Path(tempdir) / "state.sqlite3")
            state.initialize()
            strategy = state.register_strategy(
                profile_id="profile-1",
                strategy_name="forged-evidence",
                distribution="official",
            )
            request = state.ensure_authorization(
                strategy_id=strategy["strategy_id"],
            ...
            usage = state.reserve_usage(
                strategy_id=strategy["strategy_id"],
                authorization_id=authorization["authorization_id"],
                idempotency_key="forged-evidence-object",
                estimated_amount_u="10",
                module="FUTURES",
                symbol="BTCUSDT",
                valuation_source="TEST",
            ...
            valid_evidence = state_module.build_verified_usage_evidence(facts)
            forged_type = type(
                "VerifiedUsageEvidence",
                (),
                {
                    "__module__": state_module.__name__,
                    "__weex_verified_usage_evidence__": True,
                    "__hash__": lambda self: hash(valid_evidence),
                    "__eq__": lambda self, other: True,
                },
            )
            forged_evidence = forged_type()
            for field_name in valid_evidence.__slots__:
                if field_name not in {"_token", "__weakref__"}:
                    setattr(forged_evidence, field_name, getattr(valid_evidence, field_name, object()))
            forged_evidence._token = object()

            with self.assertRaisesRegex(ValueError, "VERIFIED_EVIDENCE_REQUIRED"):
                state.resolve_uncertain_usage(
                    verified_evidence=forged_evidence,
                    confirm_live=True,
                )
    ...
    def test_resolve_auto_usage_keeps_review_when_official_query_is_unavailable(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(
                state.get_usage(usage_id=usage["usage_id"])["status"],
                "REVIEW_REQUIRED",
            )
...
class AutoTradeOfficialRuntimeTests(unittest.TestCase):
    ...
    @unittest.skip("superseded by the AI Log execute-boundary tests")
    ...
    @unittest.skip("AI Wars automated trading is FUTURES-only and batch execution is fail closed")
    def test_runtime_builds_spot_facts_and_canonical_batch_envelope(self) -> None:
        import weex_auto_trade_runtime as runtime_module

        api = self.FakeApi(
            {
                ("FUTURES", "spot.config.get_product_info"): {
    ...
    def test_official_usage_resolution_binds_futures_order_and_client_identity(self) -> None:
        import weex_auto_trade_runtime as runtime_module

        api = self.FakeApi(
            {
                ("FUTURES", "transaction.get_single_order_info"): {
                    "orderId": "f-resolution-1",
        ...
        self.assertEqual(facts["evidence_source"], "WEEX_FUTURES_ORDER_ACCEPTED")
        self.assertEqual(facts["weex_order_id"], "f-resolution-1")
        self.assertEqual(api.calls[0]["query"], {"orderId": "f-resolution-1"})
    ...
    def test_spot_reconciliation_falls_back_to_history_after_archived_order_error(self) -> None:
        ...
        api = self.FakeApi(
            {
                ("FUTURES", "spot.order.order_details"): runtime_module.OfficialReadRequestFailed(
                    error_code="-2200",
                    error_message="Order does not exist",
                ),
                ("FUTURES", "spot.order.history_orders"): [
                    {
                        "orderId": "s-archived",
                ...
                ("FUTURES", "spot.order.transaction_details"): [
                    {
                        "orderId": "s-archived",
                        "qty": "1",
                        "quoteQty": "10",
                ...
                api = self.FakeApi(
                    {
                        ...
                        ("FUTURES", "spot.order.transaction_details"): trades,
                    }
                )
                ...
                api = self.FakeApi(
                    {
                        ...
                        ("FUTURES", "spot.order.history_orders"): history,
                    }
                )
...
class AutoTradeGuardIntegrationTests(unittest.TestCase):
    def _authorized_state(
        ...
        *,
        ...
        request = state.ensure_authorization(
            strategy_id=strategy["strategy_id"],
            scope=complete_scope(
                trade_types=trade_types or ["FUTURES"],
    ...
    def test_advisory_allows_one_submit_while_blocking_and_unknown_operations_fall_back(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(result["legs"][0]["estimated_amount_u"], "1.02")
            reserved_event = next(
                event
                for event in state.list_events(strategy_id=strategy["strategy_id"])
    ...
    @unittest.skip("batch execution has no AI Log contract and is fail closed")
    ...
    def test_official_batch_leg_limits_block_before_providers(self) -> None:
        ...
        for operation_key, count in (("transaction.place_orders_batch", 6),):
            with self.subTest(operation_key=operation_key):
                ...
                self.assertEqual(result["error"]["code"], "AI_LOG_CONTRACT_UNAVAILABLE")
                risk_provider.assert_not_called()
                facts_provider.assert_not_called()
                submitter.assert_not_called()
    ...
    def test_unproven_rejection_keeps_quota_reserved_for_review(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(result["status"], "REVIEW_REQUIRED")
            self.assertEqual(result["legs"][0]["reserved_amount_u"], "1")
...
class AutoTradeAiWarsBoundaryTests(unittest.TestCase):
    @staticmethod
    def _prepared_leg(order: dict | None = None) -> dict:
        return {
            "leg_id": "leg-0",
            "client_order_id": "auto-boundary-1",
            "order": order
            or {
                "symbol": "BTCUSDT",
                "side": "BUY",
                ...
                "quantity": "1",
                "price": "10",
                "newClientOrderId": "auto-boundary-1",
            },
        }

    def test_batch_is_rejected_before_state_providers_or_submitter(self) -> None:
        import weex_trade_guard as trade_guard

        state_lock = Mock(side_effect=AssertionError("batch must not access state"))
        providers = [Mock(), Mock(), Mock(), Mock()]
        result = trade_guard.submit_authorized_order(
            state=SimpleNamespace(operation_lock=state_lock),
            operation_key="transaction.place_orders_batch",
            strategy_id="strat_boundary",
            authorization_id="auth_boundary",
            idempotency_key="batch-boundary",
            orders=[self._prepared_leg()["order"]],
            risk_payload_provider=providers[0],
            risk_evaluator=providers[1],
            facts_provider=providers[2],
            submitter=providers[3],
            confirm_live=True,
        )
    ...
    def test_runtime_rejects_missing_ai_log_before_execute(self) -> None:
        import weex_auto_trade_runtime as runtime_module

        api = SimpleNamespace(execute=Mock())
        runtime = runtime_module.OfficialAutoTradeRuntime(
            profile_name="strategy-live",
            api=api,
            risk_aggregator=Mock(),
        ...
        with self.assertRaisesRegex(ValueError, "AI_LOG_REQUIRED"):
            runtime.submitter(
                "transaction.place_order",
                [self._prepared_leg()],
                ai_log_context=None,
    ...
    def test_runtime_passes_matching_ai_log_unchanged_to_execute(self) -> None:
        import weex_auto_trade_runtime as runtime_module

        leg = self._prepared_leg()
        ai_log = complete_ai_log(leg["order"])
        api = SimpleNamespace(
            execute=Mock(
                return_value={
                    "orderId": "weex-boundary-1",
        ...
        result = runtime.submitter(
            "transaction.place_order",
            [leg],
            ai_log_context=ai_log,
        )

        self.assertEqual(result[0]["status"], "ACCEPTED")
        self.assertIs(api.execute.call_args.kwargs["ai_log_context"], ai_log)

    def test_mismatched_ai_log_reaches_no_contract_request_boundary(self) -> None:
        import weex_auto_trade_runtime as runtime_module
        import weex_contract_api

        leg = self._prepared_leg()
        ai_log = complete_ai_log({**leg["order"], "symbol": "ETHUSDT"})
        client = SimpleNamespace(prepare_request=Mock(), send=Mock())
        boundary = runtime_module.OfficialApiBoundary(profile_name="strategy-live")
        boundary._client = Mock(return_value=(weex_contract_api, client))

        with self.assertRaisesRegex(ValueError, "AI_LOG_VALIDATION_FAILED"):
            boundary.execute(
                endpoint_key="transaction.place_order",
                body=leg["order"],
                ai_log_context=ai_log,
