...
from weex_order_quantity import QuantitySemanticsError, normalize_order_for_api
...
ADVISORY_DEGRADED_REASONS: frozenset[str] = frozenset()
...
def resolve_official_auto_trade_operation(operation_key: str) -> dict[str, Any] | None:
    ...
    allowed_order_fields.add("quantity_unit")
...
def _batch_ai_log_contract_unavailable() -> dict[str, Any]:
    code = "AI_LOG_CONTRACT_UNAVAILABLE"
    return _manual_fallback(
        code=code,
        blocking_reasons=[
            {
                "code": code,
                "message": "automatic batch execution has no supported AI log contract",
            }
        ],
    )
...
def submit_authorized_order(
    *,
    ...
    if operation is not None and operation["kind"] == "BATCH":
        return _batch_ai_log_contract_unavailable()
    stack = ExitStack()
    try:
...
def _submit_authorized_order_unlocked(
    *,
    ...
    for index, raw_order in enumerate(orders):
        ...
        quantity_requires_preflight = str(
            raw_order.get("quantity_unit") or raw_order.get("quantityUnit") or ""
        ).strip().upper() == "CONTRACTS"
        try:
            if quantity_requires_preflight:
                facts = facts_provider(leg)
                normalized_order, quantity_semantics = normalize_order_for_api(
                    market=operation["module"],
                    order=raw_order,
                    symbol_facts=facts.get("symbol") if isinstance(facts, dict) else None,
                )
                leg["order"] = normalized_order
                raw_order = normalized_order
            ...
            blocking_reasons.append({"code": exc.code, "message": str(exc)})
            continue
        except Exception:
            blocking_reasons.append(
    ...
    for item, reservation, order_record in zip(prepared, reservation_results, order_records):
        ...
        if outcome in {"RELEASED", "REVIEW_REQUIRED"}:
            if rejection_code_text is not None:
        ...
        for downstream_key, public_key in (
            ("trade_executed", "trade_executed"),
            ("ai_log_upload_failed", "ai_log_upload_failed"),
            ("retry_trade_safe", "retry_trade_safe"),
            ("ai_log_failure_reason", "ai_log_failure_reason"),
        ):
            if downstream_key in downstream:
                final_leg[public_key] = downstream[downstream_key]
...
def _submit_live_auto_fallback_order(
    intent: dict[str, Any],
    *,
...
def cmd_preview_order(args: argparse.Namespace, *, now_ms: int | None = None) -> int:
    ...
    order_for_intent = risk_payload.get("normalized_order")
    if not isinstance(order_for_intent, dict):
        order_for_intent = raw_order
    if _position_identity(order_for_intent, "position_id", "positionId") is not None:
        _validate_exact_position_close(order_for_intent, risk_payload)
...
def cmd_confirm_order(args: argparse.Namespace, *, now_ms: int | None = None) -> int:
    ...
    if intent.get("auto_fallback_operation_key") is not None:
        execution_payload = _submit_live_auto_fallback_order(
            intent,
            ai_log_context=ai_log_context,
        )
    else:
