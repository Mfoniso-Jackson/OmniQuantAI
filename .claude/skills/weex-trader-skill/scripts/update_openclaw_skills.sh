...
readonly DEFAULT_REPO_URL="https://github.com/weex-labs/weex-agent-skills-ai-wars"
readonly DEFAULT_BRANCH="main"
readonly APPROVED_COMMIT="1e2fb63731729c8039fa6a97108a5b0d0f6c5d7e"
readonly OPENCLAW_ROOT="${OPENCLAW_HOME:-${HOME}/.openclaw}"
readonly REPO_DIR="${WEEX_OPENCLAW_REPO_DIR:-${OPENCLAW_ROOT}/skill-repos/weex-agent-skills-ai-wars}"
...
validate_source() {
  ...
  [[ "$(canonical_repo_url "${REPO_URL}")" == "$(canonical_repo_url "${DEFAULT_REPO_URL}")" ]] \
    || fail "production updates require the official AI Wars repository"
...
validate_checkout() {
  ...
  if git -C "${checkout}" ls-files -s -- \
      skills/weex-trader-skill skills/weex-analysis-skill skills/weex-monitor-skill \
...
print_version() {
  ...
  printf 'OpenClaw WEEX AI Wars skills are ready.\n'
  printf 'Repository: %s\n' "${REPO_DIR}"
  printf 'Branch: %s\n' "${BRANCH}"
  printf 'Version: %s\n' "${version}"
