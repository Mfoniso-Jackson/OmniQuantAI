...
from weex_auto_trade_evidence_registry import (
    is_issued_evidence,
    register_issued_evidence,
)
...
def build_verified_usage_evidence(facts: dict[str, Any]) -> VerifiedUsageEvidence:
    ...
    if module != "FUTURES" or source not in _OFFICIAL_USAGE_EVIDENCE_SOURCES:
        raise ValueError("UNTRUSTED_USAGE_EVIDENCE")
    order_id = facts["weex_order_id"]
    if order_id is not None:
        order_id = _required_text(order_id, "weex_order_id")
    if outcome == "ACCEPTED":
        if order_id is None or source != "WEEX_FUTURES_ORDER_ACCEPTED":
            raise ValueError("UNTRUSTED_USAGE_EVIDENCE")
    elif outcome == "RELEASED":
        if order_id is not None or source != "WEEX_FUTURES_ORDER_NOT_FOUND":
            raise ValueError("UNTRUSTED_USAGE_EVIDENCE")
    else:
        raise ValueError("UNTRUSTED_USAGE_EVIDENCE")
    normalized = {
    ...
    evidence = VerifiedUsageEvidence(
        outcome=outcome,
    ...
    register_issued_evidence(evidence)
    return evidence


def _is_verified_usage_evidence(value: Any) -> bool:
    """Accept only the exact object issued by the trusted evidence builder."""
    return is_issued_evidence(value)
...
CREATE TABLE authorization_usage (
    ...
    module TEXT NOT NULL CHECK (module = 'FUTURES'),
...
class AutoTradeState:
        ...
        try:
            with closing(self._connect()) as connection:
                ...
                if _has_deprecated_expanded_scope(authorization):
                    connection.execute("COMMIT")
                    raise ValueError("AUTHORIZATION_SCOPE_REAUTHORIZATION_REQUIRED")
                if not authorization["futures_allowed"]:
                    raise ValueError("SCOPE_MISMATCH")
        ...
        *,
        ...
        try:
            with closing(self._connect()) as connection:
                ...
                if outcome in {"RELEASED", "REVIEW_REQUIRED"} and normalized_error_code is not None:
                    event_payload["error_code"] = normalized_error_code
                if outcome in {"RELEASED", "REVIEW_REQUIRED"} and normalized_error_message is not None:
    ...
    def resolve_uncertain_usage(
        ...
        *,
        ...
        if (
            not _is_verified_usage_evidence(verified_evidence)
            or not all(
...
def normalize_scope(scope: dict[str, Any]) -> dict[str, Any]:
    ...
    if trade_type_set != {"FUTURES"}:
        raise ValueError("trade_types only accepts FUTURES")
    trade_types = ["FUTURES"]
...
def _scope_from_row(row: sqlite3.Row) -> dict[str, Any]:
    return {
        "trade_types": [
            item
            for item, allowed in (("FUTURES", row["futures_allowed"]),)
            if allowed
        ],
