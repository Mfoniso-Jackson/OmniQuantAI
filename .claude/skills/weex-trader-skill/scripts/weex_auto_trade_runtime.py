...
from weex_order_quantity import QuantitySemanticsError, normalize_order_quantity
...
class OfficialAiLogUploadFailed(RuntimeError):
    """The primary trade succeeded, but the required AI log upload failed."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__("primary trade succeeded but AI log upload failed")
...
class OfficialApiBoundary:
    ...
    def call(
        ...
        *,
        ...
        is_trade = bool(endpoint.mutating)
        if mutating != is_trade:
            raise ValueError("official endpoint purpose does not match the requested boundary")
        if mutating:
            raise ValueError("mutating requests must use the AI Log execution boundary")
    ...
    def execute(
        ...
        *,
        ...
        api_module, client = self._client("FUTURES", private=True)
        endpoint = api_module.ENDPOINTS.get(endpoint_key)
        if endpoint is None or not endpoint.mutating:
            raise ValueError("endpoint is not a mutating official contract operation")
        try:
            exit_code, payload = api_module.execute_endpoint_payload(
                client=client,
            ...
            raise ValueError("AI_LOG_VALIDATION_FAILED") from exc
        if payload.get("businessOk") is True and (
            payload.get("aiLogUploadFailed") is True
            or exit_code == getattr(api_module, "EXIT_CODE_AI_LOG_FAILED", 2)
        ):
            ai_log_payload = payload.get("aiLog")
            failure_reason = payload.get("aiLogFailureReason")
            if failure_reason in (None, "") and isinstance(ai_log_payload, dict):
                error_payload = ai_log_payload.get("error")
                if isinstance(error_payload, dict):
                    failure_reason = error_payload.get("message")
            raise OfficialAiLogUploadFailed(
                trade_result=payload.get("result"),
                failure_reason=_sanitized_optional_official_error_text(failure_reason),
            )
        if payload.get("businessOk") is True:
            return payload.get("result")
        status = payload.get("status")
        error_payload = payload.get("result")
        error_code = _official_error_code(error_payload)
        if _is_definitive_http_rejection(status, error_code):
            raise OfficialRequestRejected(
                status=status,
        ...
        raise OfficialRequestUncertain(
            f"official WEEX write result is uncertain (exit code {exit_code})"
        )
...
class OfficialAutoTradeRuntime:
    ...
    def _futures_facts(self, order: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            quantity_semantics = normalize_order_quantity(
                market="FUTURES",
                order=order,
                symbol_facts=symbol_facts,
            )
        except QuantitySemanticsError:
            raise
        ...
        if (side, position_side) in {("SELL", "LONG"), ("BUY", "SHORT")}:
            ...
            facts["reduce_only_proven"] = _position_covers_order(
                ...
                quantity=quantity_semantics["api_quantity"],
            )
        assets = {
    ...
    def submitter(
        ...
        *,
        ai_log_context: dict[str, Any] | None,
    ) -> list[dict[str, Any]]:
        if not prepared_legs:
            raise ValueError("prepared legs are required")
        if operation_key == "transaction.place_orders_batch":
            raise ValueError("AI_LOG_CONTRACT_UNAVAILABLE")
        if ai_log_context is None:
            raise ValueError("AI_LOG_REQUIRED")
        ...
        body = orders[0]
        try:
            if module != "FUTURES" or not hasattr(self.api, "execute"):
                raise ValueError("unsupported official product module")
            payload = self.api.execute(
            ...
            return results
        except OfficialAiLogUploadFailed as exc:
            results = _normalize_submission_results(
                operation_key=operation_key,
                payload=exc.trade_result,
                ...
                result.update(
                    {
                        ...
                        "error_code": "AI_LOG_UPLOAD_FAILED",
                        "trade_executed": True,
                        "ai_log_upload_failed": True,
                        "retry_trade_safe": False,
                    }
                )
                if exc.failure_reason is not None:
                    result["error_message"] = exc.failure_reason
                    result["ai_log_failure_reason"] = exc.failure_reason
            return results
...
def query_official_usage_resolution(
    *,
    ...
    """Resolve an uncertain submission using a bound, read-only WEEX lookup.

    Futures has no supported client-order-ID lookup. Without a recorded WEEX
    order ID, the result stays unresolved instead of guessing that the order was
    rejected. A known ID must match the locally recorded client ID and order
    fields before it can produce release or acceptance evidence.
    """
    module = _required_module(order.get("module"))
    symbol = _required_text(order.get("symbol"), "symbol").upper()
    ...
    if leg_type in {"CONDITIONAL", "TAKE_PROFIT", "STOP_LOSS"}:
    ...
    if detail is None:
        return _resolution_facts(
            order,
            outcome="RELEASED",
            source="WEEX_FUTURES_ORDER_NOT_FOUND",
        )
    ...
    if returned_order_id != local_order_id:
    ...
    return _resolution_facts(
        order,
        outcome="ACCEPTED",
        source="WEEX_FUTURES_ORDER_ACCEPTED",
        weex_order_id=returned_order_id,
    )
...
def _normalize_submission_results(
    *,
    ...
    is_batch = operation_key == "transaction.place_orders_batch"
...
def _sanitized_optional_official_error_text(value: Any) -> str | None:
