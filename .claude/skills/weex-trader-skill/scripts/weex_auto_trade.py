...
class AutoTradeFacade:
    ...
    def _submit_auto(self, payload: dict[str, Any], *, confirm_live: bool) -> dict[str, Any]:
        ...
        ai_log_context = _validate_ai_log_context(payload["ai_log"])
        _validate_auto_ai_log_consistency(operation_key, orders, ai_log_context)
        ...
        if runtime is None:
            ...
            try:
                result = submit_authorized_order(
                    ...
                    submitter=lambda selected_operation, prepared: runtime.submitter(
                        selected_operation,
                        prepared,
        ...
        try:
            ...
            if current_order.get("usage_status") == "RESERVED":
                try:
                    ...
                    # Preserve the reservation if the review marker cannot be persisted.
                    pass
            raise FacadeError(
                "RECONCILIATION_FACTS_INVALID",
    ...
    intent.update(
        {
            ...
            "ai_log_context": dict(ai_log_context),
        }
    )
    return intent
...
def _validate_ai_log_context(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise FacadeError(
            "AI_LOG_REQUIRED",
            "ai_log must be a complete JSON object",
            "PROVIDE_MATCHING_AI_LOG",
        )
    try:
        from weex_ai_api import validate_ai_log_payload

        return validate_ai_log_payload(dict(value), "ai_log")
...
def _validate_auto_ai_log_consistency(
    operation_key: str,
    orders: list[Any],
    ai_log_context: dict[str, Any],
) -> None:
    """Reject a mismatched log before runtime/provider/quota work begins."""
    operation = resolve_official_auto_trade_operation(operation_key)
    if (
        operation is None
        or operation["kind"] == "BATCH"
        or len(orders) != 1
        or not isinstance(orders[0], dict)
    ):
        return
    order = orders[0]
    if set(order) - operation["allowed_order_fields"]:
        return
    if _validate_official_order_semantics(operation, order):
        return
    try:
        from weex_contract_api import build_ai_log_consistency_report

        report = build_ai_log_consistency_report(
            operation_key,
            dict(order),
            ai_log_context,
        )
        ...
        return
    if report is None or report.get("ok") is True:
        return
    ...
    raise FacadeError(
        "AI_LOG_VALIDATION_FAILED",
        "ai-log.output does not match the trade request"
