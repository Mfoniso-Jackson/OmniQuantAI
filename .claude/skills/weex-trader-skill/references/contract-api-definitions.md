...
| `symbols[].contractVal` | `Decimal` | Contract size in base-asset quantity per contract. Use it only to convert an explicit `CONTRACTS` input to base-asset quantity before submitting an order. |
...
- When only `symbol` is provided, all positions under the symbol are closed, including both long and short positions.
- When only `positionId` is provided, the corresponding position is closed.
- When both are provided, `positionId` has priority and must belong to the supplied symbol.
- When both are omitted, all open positions in the account are closed.
...
| `quantity` | `String` | `Yes` | Order quantity in base-asset units. Must be greater than 0. If the caller starts with contract counts, convert with `symbols[].contractVal` exactly once before sending; do not scale a base-asset quantity again. |
...
| `quantity` | `String` | `Yes` | Order quantity in base-asset units. Must be > 0. If the caller starts with contract counts, convert with `symbols[].contractVal` exactly once before sending; do not scale a base-asset quantity again. |
...
| `quantity` | `String` | `No` | Quantity to execute in base-asset units. Set to 0 or omit to set TP/SL for the full position. If the caller starts with contract counts, convert with `symbols[].contractVal` exactly once before sending. Copy-trading API keys must close the full position, so this field must be 0 or omitted. |
