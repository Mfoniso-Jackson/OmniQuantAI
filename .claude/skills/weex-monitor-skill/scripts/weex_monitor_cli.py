...
class DelegatedTradeReviewRequired(MonitorInputError):
    """Raised when the primary trade executed but post-trade work failed."""

    def __init__(self, payload: dict[str, Any]) -> None:
        super().__init__("delegated trade executed and must not be retried; manual review is required")
...
def _run_json_command(command: list[str]) -> Any:
    ...
    if completed.returncode != 0:
        if (
            ...
            and payload.get("tradeExecuted") is True
            and payload.get("retryTradeSafe") is False
        ):
            raise DelegatedTradeReviewRequired(payload)
        detail = (completed.stderr or completed.stdout or "").strip()
        raise MonitorInputError(f"delegated command failed ({completed.returncode}): {detail}")
    if payload is None:
        raise MonitorInputError("delegated command did not return JSON")
    return payload
...
def evaluate_order_baseline_pnl_task(
    ...
    *,
    ...
    if target is None:  # pragma: no cover - guarded by binding_error
        raise MonitorInputError("order baseline position resolution returned no target")
...
def _canonical_contract_close_output(close_order: dict[str, Any]) -> dict[str, Any]:
    if close_order.get("position_id") not in (None, ""):
        return {
            "symbol": close_order["symbol"],
            "action": "close",
            "positionId": close_order["position_id"],
        }
    return {
        "symbol": close_order["symbol"],
    ...
    *,
    ...
    return {
        ...
        "instruction": (
            ...
            f"{_confirm_flag_for_trading_mode(task['trading_mode'])} "
...
def run_live_once(
    *,
    ...
    for task in load_tasks():
        ...
        except DelegatedTradeReviewRequired as exc:
            exchange_response = _exchange_response_summary(exc.payload)
            ai_log_failure_reason = _one_line(
                str(exc.payload.get("aiLogFailureReason") or "AI Log upload failed")
            )[:500]
            review_context = {
                "trade_executed": True,
                "ai_log_upload_failed": bool(exc.payload.get("aiLogUploadFailed")),
                "retry_trade_safe": False,
                "failed_ai_log_trigger": str(exc.payload.get("failedAiLogTrigger") or ""),
                "ai_log_failure_reason": ai_log_failure_reason,
                "exchange_response": exchange_response,
                "close_order": close_order,
                "trigger_snapshot": recheck_result.get("trigger_snapshot", {}),
            }
            outputs.append(
                _mark_task_review_required(
                    task,
                    reason="trade_executed_ai_log_upload_failed",
                    detail="primary trade executed; AI Log upload failed; retry is unsafe",
                    event_type="live_order_review_required",
                    now_ms=evaluated_at_ms,
                    ...
                    thread_report=(
                        f"{_environment_prefix_for_trading_mode(task['trading_mode'])}\n"
                        f"WEEX monitor {task['task_id']} trade executed, but AI Log upload failed. "
                        f"Do not retry. Exchange summary: {exchange_response}."
                    ),
                )
            )
            continue
...
def _mark_task_review_required(
    ...
    *,
    ...
    output: dict[str, Any] = {
        ...
        "status": "review_required",
        "result": result,
        "thread_report": thread_report or render_live_thread_report(task, result, None),
    }
    if extra:
...
def _collect_live_position_confirmation(
    ...
    *,
    ...
    if binding_error is not None or target is None:
        ...
        raise MonitorInputError(f"live position confirmation failed: {reason}")
...
def _exchange_response_summary(response: Any) -> dict[str, Any]:
    ...
    field_aliases: tuple[tuple[str, tuple[str, ...]], ...] = (
        ("ok", ("ok", "success")),
        ("order_id", ("order_id", "orderId", "successOrderId", "ordId", "id")),
        (
            "client_order_id",
