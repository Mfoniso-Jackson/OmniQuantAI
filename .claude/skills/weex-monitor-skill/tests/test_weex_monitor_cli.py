...
class MonitorTaskTests(unittest.TestCase):
    ...
    def test_order_baseline_monitor_normalizes_baseline_and_fixed_quantity(self) -> None:
        ...
        self.assertEqual(
            task["baseline"],
            {"order_id": "order-1", "entry_price": "70000", "quantity": "0.01"},
        )
    ...
    def test_order_baseline_binds_unique_separated_position_by_open_order_id(self) -> None:
        ...
        self.assertEqual(result["close_order"]["quantity"], "0.001")

    def test_order_baseline_separated_position_fails_closed_without_exact_binding(self) -> None:
        base_task = {
            "task_type": "order_baseline_pnl_monitor",
            "profile": "demo",
            "trading_mode": "live",
            "symbol": "ETHUSDT",
            "position_side": "LONG",
            "baseline": {"entry_price": "2251.92", "quantity": "0.001"},
            "condition": {
                "metric": "baseline_unrealized_pnl",
                "operator": ">=",
                "threshold": "-100",
        ...
        missing_order_id = monitor.evaluate_order_baseline_pnl_task(
            base_task,
            positions,
            account_payload=account_payload,
        )
        wrong_order_task = json.loads(json.dumps(base_task))
        wrong_order_task["baseline"]["order_id"] = "missing-order"
        missing_match = monitor.evaluate_order_baseline_pnl_task(
            wrong_order_task,
            positions,
            account_payload=account_payload,
        )
        mismatched_quantity_task = json.loads(json.dumps(base_task))
        mismatched_quantity_task["baseline"].update(
            {"order_id": "open-order-1", "quantity": "0.001"}
        )
        quantity_mismatch = monitor.evaluate_order_baseline_pnl_task(
            mismatched_quantity_task,
            positions,
            account_payload=account_payload,
        )

        self.assertEqual(missing_order_id["reason"], "separated_position_order_id_required")
        self.assertEqual(missing_match["reason"], "separated_position_not_found")
        self.assertEqual(quantity_mismatch["reason"], "separated_position_quantity_mismatch")
        self.assertFalse(missing_order_id["triggered"])
        self.assertFalse(missing_match["triggered"])
        self.assertFalse(quantity_mismatch["triggered"])
    ...
    def test_position_pnl_aggregates_split_records_for_the_same_position_bucket(self) -> None:
        task = monitor.normalize_task(
            {
                "task_type": "position_pnl_monitor",
                "profile": "demo",
        ...
        positions = [
            ...
            },
            {
                ...
                "quantity": "0.01",
                "entry_price": "87000",
                "unrealized_pnl": "5.488",
            },
        ]

        result = monitor.evaluate_pnl_task(task, positions)
        reversed_result = monitor.evaluate_pnl_task(task, list(reversed(positions)))

        self.assertTrue(result["triggered"])
        self.assertEqual(result["trigger_snapshot"]["unrealized_pnl"], "11.640")
        self.assertEqual(result["close_order"]["quantity"], "0.02")
        self.assertEqual(reversed_result, result)

    def test_position_pnl_single_separated_position_uses_exact_position_id(self) -> None:
        task = monitor.normalize_task(
            {
                "task_type": "position_pnl_monitor",
                "profile": "demo",
        ...
        result = monitor.evaluate_pnl_task(task, [position])

        self.assertTrue(result["triggered"])
        self.assertEqual(result["close_order"]["position_id"], "position-1")
        self.assertEqual(result["close_order"]["quantity"], "0.01")
    ...
    def test_live_delegate_plan_carries_trading_mode(self) -> None:
        ...
        self.assertNotIn("--trading-mode", plan["instruction"])
        self.assertIn("--confirm-live", plan["instruction"])

    def test_collect_live_account_payload_uses_contract_only_aggregator_cli(self) -> None:
        task = monitor.normalize_task(
            {
                "task_type": "position_pnl_monitor",
                "profile": "demo-profile",
        ...
        self.assertNotIn("--trading-mode", command)
    ...
    def test_collect_live_account_payload_backfills_market_snapshot_price(self) -> None:
        ...
        self.assertNotIn("--trading-mode", account_command)
    ...
    def test_live_confirmation_aggregates_split_records_for_the_same_position_bucket(self) -> None:
        task_json = {
            "task_id": "mon_live_split_position",
            "task_type": "position_pnl_monitor",
            "profile": "demo",
            "trading_mode": "live",
            "symbol": "BTCUSDT",
        ...
        account_payload = {
            ...
            "account_snapshot": {"available_balance": "100"},
            "market_snapshot": {"current_price": "88000"},
            "degraded_reasons": [],
            "partial": False,
        }
        ...
        snapshot = prepared["live_position_confirmation"]
        self.assertEqual(snapshot["quantity"], "0.02")
        self.assertEqual(snapshot["unrealized_pnl"], "11.640")
        self.assertEqual(snapshot["entry_price"], "86500")
        self.assertIn("持仓数量: 0.02", prepared["confirmation_text"])
        self.assertIn("当前未实现盈亏: 11.640", prepared["confirmation_text"])
    ...
    def test_order_baseline_live_confirmation_shows_exact_separated_binding(self) -> None:
        task_json = {
            "task_id": "mon_live_separated_order_baseline",
            "task_type": "order_baseline_pnl_monitor",
            "profile": "demo",
            "trading_mode": "live",
            "symbol": "ETHUSDT",
        ...
        account_payload = {
            ...
            "market_snapshot": {"current_price": "2252"},
            "degraded_reasons": [],
            "partial": False,
        }
        ...
        text = prepared["confirmation_text"]
        self.assertIn("持仓数量: 0.001", text)
        self.assertIn("基准订单: open-order-2", text)
        self.assertIn("绑定仓位 ID: position-2", text)
    ...
    def test_run_live_once_executes_triggered_pnl_close_through_trader_guard(self) -> None:
        ...
        self.assertNotIn("--trading-mode", preview_command)
        self.assertNotIn("--trading-mode", confirm_command)
    ...
    def test_run_live_once_delegates_exact_separated_position_close(self) -> None:
        task_json = {
            "task_id": "mon_separated_order_baseline_live",
            "task_type": "order_baseline_pnl_monitor",
            "profile": "demo",
            "trading_mode": "live",
            "symbol": "ETHUSDT",
            ...
            with mock.patch.dict(os.environ, {"WEEX_MONITOR_SKILL_HOME": tempdir}, clear=False):
                ...
                with mock.patch.object(
                    ...
                    side_effect=[
                        account_payload,
                        account_payload,
                        {"intent_id": "intent-position-close", "risk_signature": "sig-position-close"},
                        {
                            "ok": True,
                            "result": [
                                {
        ...
        preview_order = json.loads(preview_command[preview_command.index("--order-json") + 1])
        self.assertEqual(results[0]["status"], "completed")
        self.assertEqual(results[0]["exchange_response"]["order_id"], "9102")
        self.assertEqual(preview_order["position_id"], "position-2")
        self.assertNotIn("new_client_order_id", preview_order)
        self.assertEqual(
            ai_log_payload["output"],
            {"symbol": "ETHUSDT", "action": "close", "positionId": "position-2"},
        )
    ...
    def test_json_delegate_surfaces_trade_executed_ai_log_failure_without_raw_retry(self) -> None:
        payload = {
        ...
        with mock.patch.object(monitor.subprocess, "run", return_value=completed):
            with self.assertRaises(monitor.DelegatedTradeReviewRequired) as exc_info:
                monitor._run_json_command(["trader", "confirm-order"])

        self.assertEqual(exc_info.exception.payload["result"][0]["successOrderId"], "9002")
        self.assertIn("must not be retried", str(exc_info.exception))

    def test_trade_executed_ai_log_failure_is_sanitized_and_review_required(self) -> None:
        task_json = {
            "task_id": "mon_pnl_ai_log_review",
            "task_type": "position_pnl_monitor",
            "profile": "demo",
            "trading_mode": "live",
            "symbol": "BTCUSDT",
        ...
        delegate_payload = {
            ...
            "aiLogUploadFailed": True,
            "retryTradeSafe": False,
            "failedAiLogTrigger": "transaction.close_positions",
            "aiLogFailureReason": "upload denied",
            "aiLog": {"raw": "raw-private-response"},
        }
            ...
            with mock.patch.dict(os.environ, {"WEEX_MONITOR_SKILL_HOME": tempdir}, clear=False):
                ...
                with mock.patch.object(
                    ...
                    side_effect=[
                        ...
                        account_payload,
                        {"intent_id": "intent-close", "risk_signature": "sig-close"},
                        monitor.DelegatedTradeReviewRequired(delegate_payload),
                    ],
                ):
                    results = monitor.run_live_once(confirm_live=True, now_ms=2000)
                tasks = monitor.load_tasks()
        ...
        self.assertEqual(result["result"]["reason"], "trade_executed_ai_log_upload_failed")
        self.assertTrue(result["result"]["triggered"])
        self.assertTrue(result["result"]["trade_executed"])
        self.assertFalse(result["result"]["retry_trade_safe"])
        self.assertEqual(result["delegate_context"]["exchange_response"]["order_id"], "9002")
        self.assertEqual(tasks[0]["status"], "review_required")
        self.assertIn("live_order_review_required", [event["event_type"] for event in events])
        persisted = json.dumps({"tasks": tasks, "events": events}, sort_keys=True)
        self.assertNotIn("raw-private-response", persisted)
