...
from omniquantai.configuration.settings import assert_exchange_execution_permitted, assert_live_trading_permitted, load_settings
...
class SettingsTests(unittest.TestCase):
    ...
    def test_exchange_execution_requires_live_mode_and_confirmations(self) -> None:
        old_enable = os.environ.pop("ENABLE_LIVE_TRADING", None)
        old_confirm = os.environ.pop("CONFIRM_REAL_MONEY", None)
        try:
            settings = load_settings()
            ...
            with self.assertRaises(PermissionError):
                assert_exchange_execution_permitted("paper", settings)

            with self.assertRaises(PermissionError):
                assert_exchange_execution_permitted("live", settings)
        finally:
            if old_enable is not None:
                os.environ["ENABLE_LIVE_TRADING"] = old_enable
            if old_confirm is not None:
